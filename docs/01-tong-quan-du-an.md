...
- **Học viên (Student):** Tìm kiếm và mua các khóa học kỹ năng nghề bằng VNĐ qua quét mã VietQR tự động (qua Webhook) hoặc thanh toán tự động qua Stripe, học theo tiến độ cá nhân, làm bài tập trắc nghiệm (Quiz) đánh giá và nhận chứng chỉ (Certificate) tự động.
...
- **Nhân viên vận hành (Staff):** Hỗ trợ vận hành hệ thống, kiểm tra lịch sử giao dịch và hỗ trợ học viên khi gặp sự cố thanh toán hoặc học tập.
...
- **Đa dạng phương thức thanh toán (Stripe & VietQR):** Tích hợp cả thanh toán quốc tế tự động qua **Stripe** (nhận thẻ Visa/Mastercard và tự động mở khóa học qua Webhook) lẫn phương thức chuyển khoản nội địa tự động bằng **quét mã VietQR** (qua Webhook kết nối cổng thanh toán ngân hàng như PayOS/SePay), đem lại sự tiện lợi tối đa cho thị trường Việt Nam.
...
- Thanh toán qua quét mã **VietQR** tự động (qua Webhook) hoặc tự động qua **Stripe** (thanh toán quốc tế)
...
- **Theo dõi giao dịch:** Xem danh sách lịch sử đơn hàng, hỗ trợ học viên giải quyết các khiếu nại liên quan đến thanh toán hoặc lỗi hệ thống.
...
#### 1. Thanh toán qua VietQR tự động qua Webhook
*   **Happy Path (Thành công):**
    Học viên chọn phương thức VietQR -> Hệ thống tạo Đơn hàng (Trạng thái: `PENDING`) -> Giao diện hiển thị mã QR động chứa thông tin chuyển khoản (Số tài khoản, Số tiền, Nội dung chuyển khoản duy nhất) -> Học viên quét mã chuyển khoản qua App ngân hàng -> Cổng thanh toán (như PayOS/SePay) nhận biến động số dư và gửi Webhook về Backend -> Backend xác thực chữ ký Webhook, mở transaction khóa dòng (`SELECT ... FOR UPDATE`):
    *   Nếu khớp thông tin và đơn hàng ở trạng thái `PENDING` -> Đơn hàng chuyển sang `PAID` -> Tự động tạo bản ghi `Enrollment` -> Cộng tiền vào ví Giảng viên (sau khi áp dụng tỷ lệ chia sẻ và khấu trừ voucher) -> Xóa giỏ hàng Redis -> Đẩy sự kiện sang Kafka để gửi email xác nhận hóa đơn qua Spring Mail -> Trả về HTTP 200 cho cổng thanh toán.
*   **Sad Path (Thất bại / Hủy):**
    Đơn hàng hết hạn thanh toán (sau 30 phút) mà hệ thống không nhận được Webhook thanh toán thành công -> Tự động chuyển thành `CANCELLED` và giải phóng voucher.
    Nếu Webhook gửi thông tin sai lệch hoặc trùng lắp -> Hệ thống kiểm tra điều kiện logic, ghi log giao dịch lỗi hoặc bỏ qua nếu đơn hàng đã được cập nhật thành công từ trước (Idempotent API).
...
- **Payment APIs:** Stripe REST API & VietQR Webhook Handler (Đối soát tự động qua Webhook).
...
| `orders` / `order_items` | Đơn hàng mua khóa học, liên kết `voucher_id` trực tiếp, lưu `payment_method` (VietQR/Stripe) và `transaction_reference` |
